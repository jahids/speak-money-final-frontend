...
const Signup: React.FC = () => {
    ...
    const handleSubmit = async () => {
		try {
			const response = await axios.post(
				"http://localhost:7000/speak/signup",
				{
                    username : username,
                    email : email,
                    password : password,
	...
	return (
		<div
			className="flex signup-parent items-center justify-center min-h-screen"
			style={{
				background:
					"linear-gradient(246deg, #ffe1bc 29.63%, #ffcfd1 51.55%, #f3c6f1 90.85%)",
			}}
		>
			<div className="relative signup  flex flex-col m-6 space-y-10 bg-white shadow-2xl rounded-2xl md:flex-row md:space-y-0 md:m-0">
				<div className="p-6 md:p-20">
					...
					<div className="flex flex-col items-center justify-between mt-6 space-y-6 md:flex-row md:space-y-0">
						<Link to="/login" className="font-light text-cyan-700">
